  ...
  public override async generateSpeakers(): Promise<void> {
    ...
    for (const row of data) {
      ...
      const speaker = {
        ...
        photo: avatar ?? 'https://drive.google.com/file/d/1mgx7EUb3gQqeYmesnBeljZVRhAE0cMsa/view?usp=sharing',
  ...
  public override async generateSessions(): Promise<void> {
    ...
    for (const row of data) {
      ...
      const speakerPromises = speakerIdsRaw.filter(Boolean).map((speakerId) => {
        return this.speakerController.getSpeaker(generateId(speakerId), this.event.id)
      })

      const [speakers, stage] = await Promise.all([
        Promise.all(speakerPromises),
        this.stageController.getStage(generateId(stageId.replace('stage_', '').replace('_', ' ')), this.event.id),
      ])
