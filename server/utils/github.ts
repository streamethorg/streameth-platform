...
export async function AddOrUpdateFile(filename: string, data: string, folder: string = DEFAULT_FOLDER) {
  ...
  try {
    const response = await fetch(`https://api.github.com/repos/${GITHUB_ORG}/${GITHUB_REPO}/contents/${folder}`, {
      method: 'PUT',
      headers: {
...
export async function GetResponseFile(filename: string, folder: string = DEFAULT_FOLDER) {
  try {
    const response = await fetch(`https://api.github.com/repos/${GITHUB_ORG}/${GITHUB_REPO}/contents/${folder}`, {
      method: 'GET',
      headers: {
...
export async function GetFile(filename: string, folder: string = DEFAULT_FOLDER) {
  try {
    const response = await fetch(`https://api.github.com/repos/${GITHUB_ORG}/${GITHUB_REPO}/contents/${filename}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${process.env.GITHUB_API_TOKEN}`,
...
export async function DeleteFile(filename?: string, folder: string = DEFAULT_FOLDER) {
  let sha = ''
  if (filename) {
    const file = await GetResponseFile(filename, folder)
    if (file) {
