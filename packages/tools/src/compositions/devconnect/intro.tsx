...
export const Intro: React.FC<
  Zod.infer<typeof DevconnectISTProps>
> = ({ type, session, id }) => {
  const { durationInFrames } = useVideoConfig()
  const frame = useCurrentFrame()

  const audioFile = staticFile(
    'devconnect/audio/IST-material-sound.mp3'
  )
  const introFile = staticFile(
    `devconnect/intro/IST-intro-${type}.mp4`
  )
          ...
          <div
            ...
            {logoFile && (
              <Sequence
                ...
                <div
                  ...
                  style={{ left: logoMove, opacity: initialOpacity }}>
            ...
            <Sequence
              ...
              <div className="absolute top-8 right-0 flex flex-col text-right text-4xl gap-4">
                <span>
                  {dayjs(session.start).format('MMMM DD, YYYY')}
                </span>
          ...
          <div
            ...
            <Sequence
              ...
              <h1
                ...
                style={{ fontFamily: 'Sofia Sans Extra Condensed' }}>
                {session.name}
              </h1>
            </Sequence>
          </div>
                  ...
                  {session.speakers.map((i) => {
                    return (
                      <div
                        ...
                        />
                        <span className="text-3xl font-medium w-48 text-center leading-normal">
                          {i.name}
                        </span>
                      </div>
                    )
