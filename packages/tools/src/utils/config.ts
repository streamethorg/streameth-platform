...
export const CONFIG = {
  ...
  DATA_FOLDER:
    process.env.DATA_FOLDER || join(process.cwd(), '../../', 'data'),
  ...
  GOOGLE_DRIVE_ID:
    process.env.GOOGLE_DRIVE_ID || '0AOwf6uTapu-fUk9PVA',
}
...
;(() => {
  ...
  if (
    ...
    writeFileSync(
      path.join(__dirname, '../../', 'google_sa_secret.json'),
      JSON.stringify(GOOGLE_SA_CREDENTIALS, null, 2)
    )
  } else {
    console.warn(
      'Google Service Account credentials are not set. Skip creating google_sa_secret.json'
    )
  }

  console.log('Create required folders at', CONFIG.ASSET_FOLDER)
  mkdirSync(join(CONFIG.ASSET_FOLDER, 'images', 'hd'), {
    recursive: true,
  })
  mkdirSync(join(CONFIG.ASSET_FOLDER, 'images', 'social'), {
    recursive: true,
  })
  mkdirSync(join(CONFIG.ASSET_FOLDER, 'intros'), { recursive: true })
  mkdirSync(join(CONFIG.ASSET_FOLDER, 'outros'), { recursive: true })
  mkdirSync(join(CONFIG.ASSET_FOLDER, 'sessions'), {
    recursive: true,
  })
