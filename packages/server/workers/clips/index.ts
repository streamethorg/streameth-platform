...
const processClip = async (data: IClip) => {
  ...
  try {
    ...
    console.log(`Found ${segments.length} segments for clip from ${start}s to ${end}s`);
    console.log('First segment starts at:', segments[0].startTime);
    console.log('Last segment starts at:', segments[segments.length - 1].startTime);
    ...
    const downloadSegment = async (segment: Segment, index: number) => {
      ...
      if (!response.ok) {
        throw new Error(`Failed to download segment at ${segment.startTime}s: ${response.statusText}`);
      }
    ...
    try {
      console.log(`Starting download of ${segments.length} segments with ${CONCURRENT_DOWNLOADS} concurrent downloads`);
      const startTime = Date.now();
      
      segmentPaths.splice(
        0,
        segments.length,
        ...(await downloadPool(segments, downloadSegment, CONCURRENT_DOWNLOADS))
      );
            ...
            .on('end', async () => {
              try {
                ...
                if (data.editorOptions) {
                  ...
                  const clipEditor = await ClipEditor.findOne({ clipSessionId: sessionId });
