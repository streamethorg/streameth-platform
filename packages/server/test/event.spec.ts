...
describe('Events API', () => {
  describe('/POST events', async () => {
    ...
    it('should return 400 when field is empty', async () => {
      ...
      await chai.request(config.testUrl).post('/events').send(event);
    });
    it('should create an event', async () => {
      ...
      response = await chai.request(config.testUrl).post('/events').send(event);
  ...
  describe('/GET events', async () => {
    it('should fetch all events', async () => {
      const res = await chai.request(config.testUrl).get('/events').send();
