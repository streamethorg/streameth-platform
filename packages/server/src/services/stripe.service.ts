...
export default class StripeService {
  ...
  private readonly PRODUCTS = {
    ...
    studio: {
      ...
      name: 'Studio Plan',
      description:
        'Enterprise features with unlimited uploads and priority support',
    },
  };
  ...
  async createCheckoutSession(
    ...
    try {
      ...
      const tierFeatures =
        this.TIER_FEATURES[tier as keyof typeof this.TIER_FEATURES];

      let productName = `StreamETH ${this.getTierNameFromPrice(data.totalPrice)}`;
      let productDescription = this.getTierDescriptionFromPrice(
        data.totalPrice,
      );
  ...
  async handleSubscriptionCreated(subscription: any): Promise<void> {
    try {
      const organization = await this.getOrganizationFromMetadata(
      ...
      const isLivestreamingEnabled =
        subscription.metadata.isLivestreamingEnabled === 'true' ||
        tierFeatures.isLivestreamingEnabled;

      const isMultistreamEnabled =
        subscription.metadata.isMultistreamEnabled === 'true' ||
        tierFeatures.isMultistreamEnabled;

      const isCustomChannelEnabled =
        subscription.metadata.isCustomChannelEnabled === 'true' ||
        tierFeatures.isCustomChannelEnabled;

      const hasPrioritySupport =
        subscription.metadata.hasPrioritySupport === 'true' ||
        tierFeatures.hasPrioritySupport;

      // Convert -1 to Infinity for unlimited values
      const finalMaxVideoLibrarySize =
        maxVideoLibrarySize === -1 ? Infinity : maxVideoLibrarySize;
  ...
  async handleSubscriptionUpdated(subscription: any): Promise<void> {
    try {
      ...
      const tier = subscription.metadata.tier || 'free';
      const maxVideoLibrarySize = Number(
        subscription.metadata.maxVideoLibrarySize || 0,
      );
  ...
  async handleCheckoutCompleted(session: any): Promise<void> {
    try {
      ...
      if (session.mode !== 'subscription') {
        console.log(
          '⚠️ Checkout is not a subscription, skipping organization update',
        );
        return;
      }
  ...
  async handleSubscriptionUpgrade(
    ...
    try {
      ...
      if (!organization) {
        console.log('⚠️ No organization found for subscription upgrade', {
          subscriptionId: subscription.id,
        });
        return;
      }
      ...
      let tier =
        subscription.metadata.tier || organization.subscriptionTier || 'free';
      ...
      try {
        ...
        console.error(
          '❌ Failed to update Stripe subscription metadata',
          error,
        );
      ...
      // Convert -1 to Infinity for unlimited values
      const finalMaxVideoLibrarySize =
        tierFeatures.maxVideoLibrarySize === -1
          ? Infinity
          : tierFeatures.maxVideoLibrarySize;
      const finalMaxSeats =
        tierFeatures.maxSeats === -1 ? Infinity : tierFeatures.maxSeats;
  ...
  async logSubscriptionDiagnostics(subscription: any): Promise<void> {
    try {
      ...
      console.log('🔍 Tier Analysis:', {
        ...
        mismatch:
          subscription.metadata?.tier !== detectedTier &&
  ...
  async handleInvoicePaymentSucceeded(invoice: any): Promise<void> {
    try {
      console.log('⚙️ Processing invoice payment succeeded event', {
        invoiceId: invoice.id,
      });
      ...
      if (
        ...
        console.log(
          '⏩ Skipping invoice - not related to a subscription with organization metadata',
        );
        return;
      }
      ...
      console.error(
        '❌ Failed to process invoice payment succeeded event',
        error,
      );
    }
  }
