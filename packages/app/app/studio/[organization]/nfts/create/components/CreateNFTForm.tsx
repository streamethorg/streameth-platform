...
const CreateNFTForm = ({
  ...
  const [isPublishingCollection, setIsPublishingCollection] = useState(false);
  ...
  const [formResponseData, setFormResponseData] = useState<INftCollection>();
  ...
  useEffect(() => {
    if (chain !== contractChainID) switchChain({ chainId: contractChainID });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [chain]);

  const parseMintFee = ethers.parseUnits(
    form.getValues('mintFee') ? (form.getValues('mintFee') as string) : '0',
    'ether'
  );
  ...
  const handleWriteContract = async () => {
    const formData: INftCollection = {
      ...
      videos: formState.selectedVideo.map((video) => ({
        type: video.videoType === 'livestreams' ? 'livestream' : 'video',
    ...
    try {
      const collectionMetadata = await generateNFTCollectionMetadataAction({
