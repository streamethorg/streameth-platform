...
const CreateLivestreamModal = ({
  ...
  const parseDate = (date?: Date, time?: string) => {
    return new Date(`${formatDate(date || new Date(), 'YYYY-MM-DD')}T${time || '00:00'}`);
  }
      ...
      {!streamType ? (
        ...
        <DialogContent className="sm:max-w-[450px] bg-white overflow-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Newly created streams are assigned a special key and RTMP ingest URL to stream into.
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)}>
              ...
              {isSchedule && (
                <>
                  <div className="mt-6">
                    <p className="text-sm" onClick={() => setIsMultiDate(!isMultiDate)}>
              ...
              <DialogFooter className="mt-8">
                ...
                <Button
                  loading={isLoading}
                  variant="primary"
                  disabled={getFormSubmitStatus(form) || isLoading || (isSchedule && isScheduleFormDisable)}
                  type="submit">
                  {!isSchedule ? 'Create' : 'Schedule'} livestream
                </Button>
              </DialogFooter>
