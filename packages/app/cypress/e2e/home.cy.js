describe('Tests for homepage', () => {
  ...
  it('Displays a list of organizations, which should have text', () => {
    cy.get(':nth-child(3) > .my-2 > a > .font-semibold').should('not.be.empty');
    cy.get(':nth-child(4) > .my-2 > a > .font-semibold').should('not.be.empty');
    cy.get(':nth-child(0) > .my-2 > a > .font-semibold').should('not.be.empty');
  });
