describe('Archive', () => {
  ...
  it('Error when no videos nor organisations have been found', () => {
    ...
    cy.get('span.bolt').contains('No videos have been found').should('exist');
  });
  ...
  it('Search for query in event', () => {
    cy.visit('/archive?event=ethberlin_2022&searchQuery=lets%20be%20fr');
    cy.get(
      '[href="/watch?event=ethberlin_2022&session=65b8f8c8a5b2d09b88ec0d4a"] > .font-semibold'
    );
  });
  ...
  it('Search for query in organization', () => {
    cy.visit('/archive?organization=ethberlin&searchQuery=lets%20be%20fr');
    cy.get(
      '[href="/watch?event=ethberlin_2022&session=65b8f8c8a5b2d09b88ec0d4a"] > .font-semibold'
    );
  });
  ...
  it('Show pagination and can click next page', () => {
    cy.intercept('GET', '/archive?searchQuery=jonas').as('searchForJonas');
    cy.visit('/archive');
